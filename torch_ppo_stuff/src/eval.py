...
def eval_moves(agent, config, num_eval=4, max_steps=500, timestep=100, seed=0, render=True, histogram=True,
         fig_callback=None):
    print("Evaluating...")
    np.random.seed(seed)
    ...
    # Compute statistics
    mean_moves = np.mean(traj_lens)
    median_moves = np.median(traj_lens)
    max_moves = max(traj_lens)
    min_moves = min(traj_lens)

    print(f"Mean Moves: {mean_moves}")
...
def eval(agent, config, num_eval=4, max_steps=500, timestep=100, seed=0, render=True, histogram=True,
         fig_callback=None):
    print("Evaluating...")
    np.random.seed(seed)
    torch.manual_seed(seed)
